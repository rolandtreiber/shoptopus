...
class OrderSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        for ($i = 0; $i < $ordersToCreate; $i++) {
            $selectedUser = User::find($userIds[random_int(1, count($userIds) - 1)]);
            ...
            $order->delivery_type_id = $deliveryTypeIds[random_int(0, count($deliveryTypeIds) - 1)];
            $order->created_at = Carbon::now()->subDays(random_int(1, 30));
            $userAddresses = $selectedUser->addresses;
            $selectedAddress = random_int(0, count($userAddresses) - 1);
            ...
            if ($hasVoucherCode) {
                ...
                $order->voucher_code_id = $voucherCodeIds[random_int(0, count($voucherCodeIds) - 1)];
            }
            $order->save();
