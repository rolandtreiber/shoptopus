...
class StripeTransactionRepository implements StripeTransactionRepositoryInterface
{
    ...
    {
        // based on environment decide what data we use
        if (config('app.env') !== 'testing') {
            ...
            Log::debug('Stripe PaymentIntent raw response', [
                'intent' => $intent->toArray(),
                'pretty' => json_encode($intent->toArray(), JSON_PRETTY_PRINT),
            ]);

            // Gard for missing latest_charge and payment_method
            $intent = $intent->toArray();
            $latestCharge = $intent['latest_charge'] ?? null;   // may be null

            if (is_null($latestCharge)) {
                throw new CheckoutException('No charge has been created yet on Intent ' . $intent['id']);
            }
        ...
        // try to format and create StripeTransaction
        try {
            ...
            throw new CheckoutException('Error try to formating the StripeTransaction class in StripeTransactionRepository: ' . $e->getMessage(),  0, $e);
        }
        ...
        if (config('app.env') !== 'testing') {
            ...
            // try to format and create PaymentSource
            try {
                if (array_key_exists('payment_method', $transaction) && array_key_exists('payment_method_details', $transaction)) {
                    ...
                    // TODO: in future implement other then card as well extend the create payment intend so stripe will pick up not just card payments
                }
            } catch (\Throwable $e) {
                throw new CheckoutException('Error try to formating the PaymentSource class in StripeTransactionRepository: ' . $e->getMessage(),  0, $e);
            }

            // try to format and create Payment
            try {
                $payment = new Payment();
                ...
                throw new CheckoutException('Error try to formating the Payment class in StripeTransactionRepository: ' . $e->getMessage(),  0, $e);
            }
        }
