...
class SharedOptionsResource extends JsonResource
{
    ...
    {
        $categories = ProductCategoryNestedTreeResource::collection(ProductCategory::availability(AvailabilityStatus::Enabled)->whereNull('parent_id')->where('enabled', 1)->select('id', 'name', 'slug', 'menu_image', 'header_image')->with('children.children.children.children.children.children')->get());
